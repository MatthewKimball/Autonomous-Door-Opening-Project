    ...
    : jaco_comm_(arm), node_handle_(nodeHandle)
{
    ...
    elevator_button_subscriber_ = node_handle_.subscribe("in/elevator_button", 1,
                                                          &JacoArm::elevatorButtonCallback, this);
    door_open_subscriber_ = node_handle_.subscribe("in/door_open", 1,
                                                          &JacoArm::dooropenCallback, this);
    calibration_subscriber_ = node_handle_.subscribe("in/kinect_arm_calibration", 1, &JacoArm::calibrationCallback, this);
...
/*
*********************************************************************************************************
*                                           Calibration Subscriber Callback Function
*********************************************************************************************************
*/
void JacoArm::calibrationCallback(const geometry_msgs::Vector3ConstPtr& calibrate)
{
    if (!jaco_comm_.isStopped())
    {

        kinect_position_.X = calibrate->x;
        kinect_position_.Y = calibrate->y;
        kinect_position_.Z = calibrate->z;

        manipulate.Coordinate_Calibration(kinect_position_);
    }
}
/*
*********************************************************************************************************
*                                              Elevator Button Subscriber Callback Function
*********************************************************************************************************
*/
void JacoArm::elevatorButtonCallback(const geometry_msgs::TwistStampedConstPtr& cartesian_vel)
{
    if (!jaco_comm_.isStopped())
    {
        cartesian_position_.X = cartesian_vel->twist.linear.x;
        cartesian_position_.Y = cartesian_vel->twist.linear.y;
        cartesian_position_.Z = cartesian_vel->twist.linear.z;
        cartesian_position_.ThetaX = cartesian_vel->twist.angular.x;
        cartesian_position_.ThetaY = cartesian_vel->twist.angular.y;
        cartesian_position_.ThetaZ = cartesian_vel->twist.angular.z;

	manipulate.elevatorButton(cartesian_position_);
    }
}

/*
*********************************************************************************************************
*                                           Door Open Button Subscriber Callback Function
*********************************************************************************************************
*/
void JacoArm::dooropenCallback(const jaco_msgs::DoorOpenConstPtr& door_characteristics)
{
    if (!jaco_comm_.isStopped())
    {
        door.cartesian_handle_position_.X = door_characteristics->handle_axis_rotation.x;
        door.cartesian_handle_position_.Y = door_characteristics->handle_axis_rotation.y;
        door.cartesian_handle_position_.Z = door_characteristics->handle_axis_rotation.z;

        door.cartesian_hinge_position_.X = door_characteristics->door_hinge.x;
        door.cartesian_hinge_position_.Y = door_characteristics->door_hinge.y;
        door.cartesian_hinge_position_.Z = door_characteristics->door_hinge.z;

        door.handle_length_ = door_characteristics-> handle_length;
        door.handle_type_ = door_characteristics-> handle_type;
        door.opening_direction_ = door_characteristics-> hinge_direction;

        manipulate.DoorOpen(door);
    }
}
